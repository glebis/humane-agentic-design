---
name: brandkit
description: This skill should be used when EXPLORING a brand identity that does not exist yet — "brand directions", "identity candidates", "brand boards", "logo system", "brand kit", "визуальные направления бренда". Generates premium brand-guidelines boards for competing directions, and hands the WINNING direction off into the design-tokens brand block (confirm-then-write). Upstream of tokens; for producing assets under an EXISTING token contract use brand-illustrate instead. Trained for minimalist, cinematic, editorial, dark-tech, luxury, cultural, security, gaming, developer-tool, and consumer-app brand systems. Optimized for intentional logo concepting, refined composition, sparse typography, strong symbolic meaning, premium mockups, art-directed imagery, and flexible grid layouts.
---
...
# DIRECTION EXPLORATION & HANDOFF (humane cycle integration)

brandkit is the upstream twin of `brand-illustrate`: it explores identity BEFORE
tokens exist; brand-illustrate produces assets AFTER. Do not use brandkit to
generate assets for a brand that already has a token contract.

**Flow:**
1. From the brief (or the project's jtbd.json, if present), propose 2–3 DISTINCT
   direction candidates — different worlds, not variations of one.
2. Generate one board per direction (draft budget) via whichever image backend is
   installed (`scripts/brandkit.py backends`); boards follow the panel DNA above.
3. The user picks a winner. Extract the direction's `imageryStyle` (prose),
   `mood` (adjectives), `avoid` (negatives), `subjects` — present them for
   CONFIRMATION (structured-choice tool when the agent has one; plain questions
   otherwise). Never write without confirmation.
4. On confirm, run the handoff:

```bash
python3 scripts/brandkit.py handoff --direction direction.json --tokens <set>.tokens.json   # existing set: writes $extensions, regenerates DESIGN.md
python3 scripts/brandkit.py handoff --direction direction.json --out-dir <project-dir>      # no set yet: saves brand-block.draft.json
```

A saved draft is imported automatically by `humane:design-tokens` `setup-edit`
run in the same directory — the new token set is born with its art direction.
