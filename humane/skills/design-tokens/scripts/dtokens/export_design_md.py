...
# Stale-overwrite marker: a DESIGN.md that carries this line was produced by us
# and is safe to regenerate; one without it was hand-written or foreign — warn.
GENERATED_MARKER = f'generator: {_yq(GENERATOR)}'
...
def _frontmatter(name, description, colors, typography, rounded, spacing,
    ...
    # Provenance (skill convention): this file is a compiled render of the token
    # set, not a hand-edited document. Emitted only when the caller supplies the
    # source; keeps the frontmatter otherwise standard for Labs consumers.
    if source:
        lines.append(GENERATED_MARKER)
        lines.append(f"source: {_yq(source)}")
        if command:
            lines.append(f"regenerate: {_yq(command)}")
    if colors:
        lines.append("colors:")
        for k in colors:
...
def _body(name, description, colors, typography, rounded, spacing, skipped,
          descs=None, color_vars=None, spacing_vars=None, source=None, command=None):
    descs = descs or {}
    ...
    if source:
        regen = f"`{command}`" if command else "the design-tokens compiler"
        lines += ["", (f"_Generated by {GENERATOR} from `{source}` — do not hand-edit. "
                       f"Change the token set and regenerate with {regen}._")]
...
def _rich_sections(brand, resolved):
    ...
    # imageryStyle is surfaced by the default Brand direction section; the rich
    # Imagery section is only for extended `imagery` prose when explicitly set.
    if brand.get("imagery"):
        lines += ["", "## Imagery", "", str(brand["imagery"])]
    if brand.get("layout"):
...
def _brand_direction(brand):
    """Default-output section rendering the art-direction contract (mood,
    imageryStyle, subjects, avoid) from the $extensions brand block. Empty list
    if the block carries none of these keys."""
    ...
    rows = []
    if brand.get("mood"):
        rows.append(("Mood", _join(brand["mood"])))
    if brand.get("imageryStyle"):
        rows.append(("Imagery", _join(brand["imageryStyle"])))
    if brand.get("subjects"):
        rows.append(("Subjects", _join(brand["subjects"])))
    if brand.get("avoid"):
        rows.append(("Avoid", _join(brand["avoid"])))
    if not rows:
        return []
    return ["", "## Brand direction", ""] + [f"- **{label}:** {val}" for label, val in rows]
...
def to_design_md(resolved, name, description=None, brand=None, rich=False,
    ...
    Provenance frontmatter + a do-not-edit line are emitted when `source` is
    given (the file is a compiled render, not hand-edited). A **Brand direction**
    section (mood / imageryStyle / subjects / avoid) renders in the DEFAULT output
    whenever the $extensions brand block carries those keys. With rich=True, also
    append the extended skill-convention sections (components, do's/don'ts,
    surfaces, layout, similar brands, Quick Start CSS) — no longer plain Labs alpha.
    """
    ...
    lines += _brand_direction(brand or {})
    if rich:
