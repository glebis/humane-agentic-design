...
Use the `linear` CLI on your `PATH` (in Claude Code it ships as a `linear` skill script — see "Claude Code extras" below). Pass the team with `--team <your-team>`. Prefer `--description-file` style by passing markdown via `-d`. One command per task:

```bash
linear create "[H6] Toolbar icons unlabeled — recognition burden" \
  --team "<your-team>" \
  --priority high \
  --label usability \
...
In Claude Code the Linear CLI is provided by the separate `linear` skill; its script lives at `<claude-config>/skills/linear/scripts/linear` and you can call that path directly if `linear` is not on your `PATH`. On other agents, install any Linear CLI and expose it as `linear`. The field mapping and the propose-then-confirm gate above are the same regardless of how the binary is invoked.
